...
namespace TechArtPlayground.Cloth
{
    public class ComputeClothSim : MonoBehaviour
    {
        ...
        [Header("Cloth Settings")] public Vector2Int resolution = new(32, 32);

        public Vector2 dimensions = new(5f, 5f);

        [Header("Prayer Flag Mode")]
        [Tooltip("If true, slices the cloth into vertical ribbons attached to a main top rope.")]
        public bool isPrayerFlagMode;

        [Tooltip("How many vertices wide each flag should be.")]
        public int flagWidth = 5;

        [Tooltip("1.0 is a tight straight line. 0.0 is a loose, deep sagging curve.")] [Range(0.0f, 10.0f)]
        public float ropeTension = 0.5f;

        [Header("Physics Parameters")] public Vector3 gravity = new(0, -9.81f, 0);

        public Vector3 windVelocity = new(2f, 0, 5f);
        ...
        private void InitializeSimulation()
        {
            ...
            List<SpringLink> springLinks = new(_vertexCount);
            List<Spring> springs = new();

            Vector2 step = new(dimensions.x / (resolution.x - 1), dimensions.y / (resolution.y - 1));
            ...
            for (int x = 0; x < resolution.x; x++)
            {
                ...
                float uCoord = isPrayerFlagMode
                    ? (float)(x % flagWidth) / (flagWidth - 1)
                    : (float)x / (resolution.x - 1);
            ...
            List<Int3> triangles = new(_triangleCount);
            List<int> meshIndices = new(_triangleCount * 3);
            ...
            _dummyMesh.bounds = new Bounds(transform.position,
                new Vector3(dimensions.x * 5, dimensions.y * 5, dimensions.x * 5));
