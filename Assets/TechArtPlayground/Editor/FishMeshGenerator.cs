...
namespace TechArtPlayground.Editor
{
    ...
    {
        [Header("Dimensions du poisson")] public float fishLength = 1.0f;
        ...
        [Header("Nageoire caudale (Queue)")] public float tailLength = 0.3f;
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.Space();
            if (GUILayout.Button("Générer et Sauvegarder le Modèle", GUILayout.Height(40))) GenerateMesh();
        }
        ...
        private void GenerateMesh()
        {
            ...
            triangles.AddRange(new[] { 0, 2, 1, 0, 3, 2, 0, 4, 3, 0, 1, 4 });
            ...
            for (int r = 0; r < numRings - 1; r++)
            {
                ...
                triangles.AddRange(new[] { c, n + 1, n, c, c + 1, n + 1 }); // Top-Right
                triangles.AddRange(new[] { c + 1, n + 2, n + 1, c + 1, c + 2, n + 2 }); // Right-Bot
                triangles.AddRange(new[] { c + 2, n + 3, n + 2, c + 2, c + 3, n + 3 }); // Bot-Left
                triangles.AddRange(new[] { c + 3, n, n + 3, c + 3, c, n }); // Left-Top
            }

            // Dernier anneau vers base de la queue
            int lastRing = 1 + (numRings - 1) * 4;
            triangles.AddRange(new[] { tailBaseIndex, lastRing, lastRing + 1 });
            triangles.AddRange(new[] { tailBaseIndex, lastRing + 1, lastRing + 2 });
            triangles.AddRange(new[] { tailBaseIndex, lastRing + 2, lastRing + 3 });
            triangles.AddRange(new[] { tailBaseIndex, lastRing + 3, lastRing });

            // Nageoire (Double face)
            triangles.AddRange(new[] { tailBaseIndex, finTopIndex, finBotIndex }); // Face Droite
            triangles.AddRange(new[] { tailBaseIndex, finBotIndex, finTopIndex }); // Face Gauche
